...
require_once '../utilities/header.php'; ?>
    ...
    if (isset($_GET['view']) || isset($_GET['view1'])) {
        ...
        if (is_null($valeurResume[htmlspecialchars($_GET[$view])]['villeEtape']) || $valeurResume[htmlspecialchars($_GET[$view])]['villeEtape'] == ";") {
            $texte .= ' Direct' ;
        } else {
                            ...
                            while($i-- > 0) { ?>
                                <tr>
                                    ...
                                    if ($valeurTrajet[$keys[$i]]['idConducteur'] == $valeur['mailUtilisateur']) { ?>
                                        <td><a href='../trajets/trajetViewProposer.php?dupl=<?php echo $keys[$i];?>'><span class="glyphicon glyphicon-plus"></span></a>
