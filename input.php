...
function affichePassagers($champPassagers) {
    $passagers = explode(";",$champPassagers);
    foreach ($passagers as &$value) {
        if ($value != "") {
            $value = affichePrenomNom($value);
        }
    }
    return $passagers;
}

function afficheEtapes($champEtapes) {
    $etapes = explode(";",$champEtapes);
    $null = array_pop($etapes);
    return $etapes;
}
